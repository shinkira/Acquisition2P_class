...
castType = 'single';
sliceNum = 1;
channelNum = FOV2.motionRefChannel;
mov = readCor(FOV2,movNum,castType,sliceNum,channelNum);
rawMov = readRaw(FOV2,movNum,castType);
...
FOV1.calcPxCov(movNums,radiusPxCov,temporalBin,sliceNum,channelNum,writeDir);

% Now we will create the binary file 'indexed movie' for the same
% slice and channel. 

FOV1.indexMovie(sliceNum,channelNum,writeDir);
...
sliceNum = 1;
actImg = FOV1.roiInfo.slice(sliceNum).covFile.activityImg;
% img = img/2 + actImg/2;
...
FOV1.selectROIs(img,sliceNum,channelNum,smoothWindow,excludeFrames);
...
%Now get traces
[dF,traces,rawF,roiList] = extractROIsBin(FOV1,roiGroups,sliceNum,channelNum);
...
sliceNum = 1;
channelNum = 1;
FOV1.calcSeedCov([],[],[],[],sliceNum,channelNum);
FOV1.indexMovie(nSlice,nChannel);
FOV1.save,

FOV1.selectROIs([],sliceNum,channelNum);
FOV1.save,
