...
for si = 1:nSlices
    ...
    pick_frame = false(1,totalFrames);

    if length(SI_time_stamp_ms) >= totalFrames;
        pick_frame(si,sliceNum:nSlices:totalFrames) = true;
    ...
    if NI_time_stamp_ms(end) < SI_time_stamp_ms(end);
        ...
        pick_beforeVRend = false(1,totalFrames);
...
save([obj.defaultDir,save_name],'TM','dF','pick_frame');
